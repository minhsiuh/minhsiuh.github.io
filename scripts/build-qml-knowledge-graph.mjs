...
const pubsPath = new URL('../data/research-qml.publications.json', import.meta.url);
const collabPath = new URL('../data/research-qml.collaborators.json', import.meta.url);
const outGraphPath = new URL('../data/research-qml.knowledge-graph.json', import.meta.url);
const outCsvPath = new URL('../data/research-qml.collab-matrix.csv', import.meta.url);
...
for (const g of pubs.groups || []) {
  for (const p of g.publications || []) {
    ...
    const c = (p.venue || '').match(/·\s*(\d+)\s+citations/i);
    const citations = c ? Number(c[1]) : null;
...
const graph = {
  version: '1.0',
  topic: pubs.topic || 'Quantum Machine Learning',
...
console.log('✅ Built qml knowledge graph + matrix');
