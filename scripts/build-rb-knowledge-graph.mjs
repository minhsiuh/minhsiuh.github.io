...
const pubsPath = new URL('../data/research-rb.publications.json', import.meta.url);
const collabPath = new URL('../data/research-rb.collaborators.json', import.meta.url);
const outGraphPath = new URL('../data/research-rb.knowledge-graph.json', import.meta.url);
const outCsvPath = new URL('../data/research-rb.collab-matrix.csv', import.meta.url);
...
for (const g of pubs.groups || []) {
  for (const p of g.publications || []) {
    const y = (p.venue || '').match(/(19|20)\d{2}/g);
    const year = y ? Number(y[y.length - 1]) : null;
    const c = (p.venue || '').match(/(\d[\d,]*)\s+citations?/i);
    const citations = c ? Number(c[1].replace(/,/g, '')) : null;
...
const institutions = (collab.institutions || []).map((i, idx) => ({
  institutionId: `I${String(idx + 1).padStart(3, '0')}`,
  name: i.name,
  country: i.country,
  lat: i.location?.[0] ?? null,
  lon: i.location?.[1] ?? null
}));
const instIdByName = new Map(institutions.map(i => [i.name, i.institutionId]));

const collabSet = new Set(collab.collaborators || []);
const collaborators = [...collabSet]
  .sort((a, b) => {
...
for (const c of collaborators) {
  ...
  c.affiliations = insts.map((i, idx) => ({
    institutionId: instIdByName.get(i.name) || null,
    institution: i.name,
    country: i.country,
    startYear: null,
    endYear: null,
...
for (const p of papers) {
  ...
  for (const a of src.authors) {
    ...
    edges.push({
      paperId: p.paperId,
      collaboratorId: cid,
      institutionId: inst ? instIdByName.get(inst.name) : null,
      role: 'coauthor'
    });
    const co = collabObjByName.get(a);
    if (co && !co.paperIds.includes(p.paperId)) co.paperIds.push(p.paperId);
  }
}
...
const graph = {
  version: '1.0',
  topic: pubs.topic || 'Randomized Benchmarking',
...
console.log('✅ Built rb knowledge graph + matrix');
