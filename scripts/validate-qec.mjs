...
const html = fs.readFileSync(new URL('../research-qec.html', import.meta.url), 'utf8');
const pubs = JSON.parse(fs.readFileSync(new URL('../data/research-qec.publications.json', import.meta.url), 'utf8'));
const collab = JSON.parse(fs.readFileSync(new URL('../data/research-qec.collaborators.json', import.meta.url), 'utf8'));
...
if (!html.includes('data/research-qec.publications.json')) errors.push('HTML not wired to qec publications JSON');
if (!html.includes('data/research-qec.collaborators.json')) errors.push('HTML not wired to qec collaborators JSON');
...
if (errors.length) {
  console.error('❌ validate-qec failed');
  errors.forEach(e => console.error(' -', e));
  process.exit(1);
}

console.log('✅ validate-qec passed');
